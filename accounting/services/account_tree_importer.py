...
class AccountTreeImporter:
    ...
    def parse_file(self) -> Dict:
        ...
        while i < len(lines):
            ...
            if 'گروه اول' in line or ('گروه 1' in line and 'دارایی های جاری' in line):
                ...
                # فرمت: "گرو ه X که نام آن ... است"
                match = re.search(r'گرو\s*ه\s*(\d+)\s*که\s*نام\s*آن\s*(.+?)\s*است', line)
                if match:
                    group_match = {'code': match.group(1), 'name': match.group(2).strip().rstrip(':').rstrip()}
            elif re.search(r'^گروه\s*(\d+)', line):
                # فرمت: "گروه X که نام آن ... است"
            ...
            gl_match = None
            # فرمت: "نام 31که شامل" - عدد 2 رقمی که بعدش "که" آمده
            ...
            # تشخیص معین
            # فرمت: "صندوق کد 1101" یا "بانکها  کد 1102" یا "شرکتها ،موسسات و سازمانها  کد 1501"
            # یا "نرم افزار های مالی 2401" یا "ساختمان های اداری5601" (بدون فاصله)
            # یا "حق الزحمه پرداختنی کارشناس رسمی3901" (بدون فاصله)
            # یا "3401 مالیات پرداختنی سال" (کد در ابتدا)
            # یا "اندوخته صرف سهام افزایش سرمایه ... 5302" (با نقطه‌های اضافی)
            sub_match = None
            # فرمت با "کد"
            match_with_kod = re.search(r'^(.+?)\s+کد\s+(\d{4})\s*$', line)
            if match_with_kod:
                sub_match = match_with_kod
            else:
                ...
                if match_code_first:
                    ...
                    if match_without_kod:
                        # بررسی اینکه عدد 4 رقمی است و خط قبلی "که شامل" نیست
                        prev_line = lines[i - 1].strip() if i > 0 else ""
                        # همچنین بررسی کنیم که این حساب کل نباشد (2 رقم)
                        if (not prev_line.startswith('که شامل') and 
                            not prev_line.startswith('حساب های کل') and
                            len(match_without_kod.group(2)) == 4):
    ...
    def create_or_get_group(self, group_code: str, group_name: str) -> AccountGroup:
        """گروه را ایجاد می‌کند یا برمی‌گرداند"""
    ...
    def create_or_get_gl_account(self, account_code: str, account_name: str, group_code: str) -> Account:
        """حساب کل را ایجاد می‌کند یا برمی‌گرداند (یا update می‌کند اگر تفصیلی نداشته باشد)"""
        ...
        try:
            ...
            # اگر حساب تفصیلی (معین) دارد، skip می‌کنیم
            if self.has_tafsili_children(gl_account):
                self.stats['gl_accounts_skipped'] += 1
            else:
                # اگر تفصیلی ندارد، می‌توانیم update کنیم
                ...
                gl_account.save()
                self.stats['gl_accounts_created'] += 1  # به عنوان updated در نظر می‌گیریم
    ...
    def create_or_get_sub_account(self, account_code: str, account_name: str, gl_code: str) -> Account:
        """حساب معین را ایجاد می‌کند یا برمی‌گرداند (یا update می‌کند اگر تفصیلی نداشته باشد)"""
        # دریافت حساب کل
        ...
        try:
            ...
            # اگر حساب تفصیلی دارد، skip می‌کنیم
            if self.has_tafsili_children(sub_account):
                ...
                sub_account.save()
                self.stats['sub_accounts_created'] += 1  # به عنوان updated در نظر می‌گیریم
        except Account.DoesNotExist:
